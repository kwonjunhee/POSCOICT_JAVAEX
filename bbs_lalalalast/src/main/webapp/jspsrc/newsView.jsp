...
<body>
	...
	List<NewsVO> list = new NewsDAO().newsListAll((page1-1)*look, look);
	...
	<% if(session.getAttribute("loginSession")!=null) {
		...
		<tr>
			...
			<td><%=vo.getCnt()%></td>
			<td><%=vo.getGood()%>&sol;<%=vo.getBad()%></td>
		</tr>
		<%}%>
	</table>
	...
	<% } %>
	<%	int n = (int) (dao.getCount() / look + 1);
		for (int i = 1; i <= n; i++) { %>
			<a href="/bbs/news?pageNumber=<%=i%>&looknum=<%=look%>">[<%=i%>]</a>
	<% } %>
	<div class="container">
		<div class="row">
			<form method="get" action="/bbs/news">
