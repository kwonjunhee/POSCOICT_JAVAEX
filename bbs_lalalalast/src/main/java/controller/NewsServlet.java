...
public class NewsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		} else if (action != null && action.equals("good")) { // 제목 선택 시 : 해당 뉴스 id 로 해당 뉴스 내용 출력(id)
			NewsVO selectedNewsVO = dao.newsListOne(Integer.parseInt(id));
			dao.good(selectedNewsVO);
			target = "/jspsrc/newsViewOne.jsp";
			request.setAttribute("selectedNewsVO", selectedNewsVO);
			System.out.println("글을 추천합니다.");
		} else if (action != null && action.equals("bad")) { // 제목 선택 시 : 해당 뉴스 id 로 해당 뉴스 내용 출력(id)
			NewsVO selectedNewsVO = dao.newsListOne(Integer.parseInt(id));
			dao.bad(selectedNewsVO);
			target = "/jspsrc/newsViewOne.jsp";
			request.setAttribute("selectedNewsVO", selectedNewsVO);
			System.out.println("글을 비추천합니다.");
		} else {
			int lnum = (Integer.parseInt(looknum));
			int page = (Integer.parseInt(pageNumber)-1)*lnum;
