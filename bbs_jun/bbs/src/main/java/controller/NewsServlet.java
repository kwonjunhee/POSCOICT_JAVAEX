...
public class NewsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String target = "/jspsrc/newsView.jsp?pageNumber=1";
		...
		if (action != null && action.equals("delete")) { // 삭제 버튼 클릭시 : 뉴스 삭제(id)
			...
			request.setAttribute("list", dao.getSearch(searchField, searchText));
			...
			if(pageNumber==null){
				List<NewsVO> list = dao.newsListAll(0);
				request.setAttribute("list", list);
			} else {
				List<NewsVO> list = dao.newsListAll((Integer.parseInt(pageNumber)-1)*5);
				request.setAttribute("list", list);
